...
_STOP = set(
    ...
    "what when where which who how part where's there here then than very".split()
)
...
def keywords(request: str) -> set[str]:
    words = {w for w in _WORD.findall(request.lower()) if w not in _STOP and len(w) > 2}
    ...
    for _ in range(max_clips):
        ...
        for i in range(len(cues)):
            ...
            for j in range(i, len(cues)):
                ...
                # prefer shorter windows with the same score: mild length penalty
                w_score = total - 0.02 * dur
